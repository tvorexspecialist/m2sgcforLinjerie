...
function updateProductsInCart (request, updateItems, cartId, accessToken, cartUrl, log, rejectUnauthorized, cb) {
  ...
  request.post(options, (err, res) => {
    ...
    log.debug({duration: new Date() - requestStart, statusCode: res.statusCode, request: util.inspect(options, true, null), response: util.inspect(res.body, true, null)}, 'updateProductsInCart response')
    cb()
  })
}
