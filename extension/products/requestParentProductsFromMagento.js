...
async function requestParentProductFromMagento (request, productId, accessToken, url, log, rejectUnauthorized) {
  ...
  log.debug({duration: new Date() - requestStart, statusCode: res.statusCode, request: util.inspect(options, true, null), response: util.inspect(res.body, true, null)}, 'Request to Magento: requestParentProductsFromMagento')
  return res.body
}
